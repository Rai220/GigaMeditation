...
def _generate_text(topic, backgound):
    return model(
        [
            HumanMessage(
                content=f"Придумай текст для медитации. Медетирующий будет слышать {backgound}. Тема медитации - {topic}. Текст должен быть расслабляющий и успокаивающий. Не пиши никаких пояснений к тексту."
            )
        ]
    ).content.replace("\n", "")
